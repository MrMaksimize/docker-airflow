...
claims <- read.csv("TSW_geoc.csv", header=T, na.strings=c(""," ","NA"))
...
test<-setDT(claims)[, .(PAID_TOTAL= sum(PAID_TOTAL, na.rm = TRUE), PAID_BIPD= sum(PAID_BI + PAID_PD, na.rm = TRUE),
                          PAID_EXP=sum(PAID_EXPENSE),Count=n_distinct(CLAIM_NUMBER)), by = .(Fiscal_Year)]
